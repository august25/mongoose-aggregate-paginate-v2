...
function aggregatePaginate(query, options, callback) {
  ...
  const qCacheName = hash(q)
  const countCacheName = hash(countQuery)
  const cacheOptions = options.cacheOptions || { name: 'paginate-agg-', time: 0 }

  if (isPaginationEnabled) {
    q.skip(skip).limit(limit);
  }

  if (typeof q.cache === "function") {
    q = q.cache(cacheOptions.time, cacheOptions.name + qCacheName)
  }

  if (typeof countQuery.cache === "function") {
    countQuery = countQuery.cache(cacheOptions.time, cacheOptions.name + countCacheName)
  }
